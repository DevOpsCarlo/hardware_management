document.addEventListener("DOMContentLoaded", function () {
  ...
  const assetTable = document.getElementById("assetTable"); // Get the table element
  ...
  function updateButtonStates() {
    setTimeout(() => {
      ...
      const hasUnassignedAssets = Array.from(checkedAssets).some(
        (asset) => asset.dataset.status?.trim() === "Available"
      );
      const hasBranchAssignedAssets = Array.from(checkedAssets).some(
        (asset) => asset.dataset.status?.trim() === "Branch Assigned"
      );
      const hasEmployeeAssignedAssets = Array.from(checkedAssets).some(
        (asset) => asset.dataset.status?.trim() === "Employee Assigned"
      );
  ...
  toggleEmployeeModal.addEventListener("click", () => {
    ...
    const branchCell = firstAssetRow.querySelector("td:nth-child(8)");
  ...
  // ============================================
  // This handles checkboxes on all pages (including new ones added by DataTables)
  assetTable.addEventListener("change", function (e) {
