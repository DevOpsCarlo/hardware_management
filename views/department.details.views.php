    ...
    <div class="fixed inset-0 top-0 left-0 bg-black/30 backdrop-blur-xs flex items-center justify-center z-50 hidden" id="department-modal">
      <div class="max-w-xl w-full mx-auto rounded-sm shadow-lg p-4 bg-white" id="department-modal-form-box">
        ...
        <form class="space-y-4 bg-white p-6 text-slate-800 grid grid-cols-1 gap-2 form text-sm font-medium" method="POST" id="department-modal-form">
          <!-- Department -->
          <input type="hidden" name="branchId" value="<?= htmlspecialchars($branch['branch_id'] ?? 0) ?>">
          <input type="hidden" name="departmentId" id="department-id" value="<?= $branch['id'] ?? 0 ?>">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ">
      <?php if (!empty($departments)): ?>
        <?php foreach ($departments as $department): ?>
          <div class="bg-white rounded-md shadow-md hover:shadow-lg transition-shadow duration-200 border border-slate-50">
            <div class="p-6">
                  ...
                  <div class="absolute top-6 right-0 w-32 bg-white border rounded shadow-lg z-10 hidden department-menu" id="department-menu-<?= $department['id'] ?>">
                    <ul class="text-xs text-slate-700 font-light">
                      <li class="px-4 py-2 hover:bg-slate-100 border-b">
                        <button class="cursor-pointer w-full text-left edit-department-btn"
                          data-department-id="<?= $department['id'] ?>"
                          data-department-name="<?= $department['department_name'] ?>"
                          data-department-head="<?= $department['department_head'] ?>">
                          Edit
                        </button>
                      </li>
                      <li class="px-4 py-2 hover:bg-slate-100">
                        <button class="cursor-pointer w-full text-left delete-department-btn"
                          data-department-id="<?= $department['id'] ?>"
                          data-department-name="<?= $department['department_name'] ?>"
                          data-department-head="<?= $department['department_head'] ?>">
                          Delete
            ...
            <div class="bg-slate-50 px-6 py-3 border-t">
              <a href="/branch/<?= urlencode($branch['branch_name']) ?>/<?= urlencode($department['department_name']) ?>" class="text-red-600 hover:text-red-700 text-sm font-medium w-full text-left cursor-pointer">
